...
class MoodProvider with ChangeNotifier {
  ...
  final Map<String, List<String>> _insightVariations = {
    '😊': [
      "You've been mostly happy! Keep doing what you're doing. Try to note down what triggered these good vibes.",
      "The sun is shining on you! Your mood has been consistently positive. Spread that joy!",
  ...
  int get currentStreak {
    ...
    final sortedEntries = List<MoodEntry>.from(_entries)..sort((a, b) => b.timestamp.compareTo(a.timestamp));
    int streak = 0;
    ...
    for (var entry in sortedEntries) {
      ...
      if (entryDate == lastDate) {
        if (!foundToday) { streak++; foundToday = true; }
      } else if (entryDate == lastDate.subtract(const Duration(days: 1))) {
        streak++; lastDate = entryDate;
      } else if (entryDate.isBefore(lastDate.subtract(const Duration(days: 1)))) { break; }
    }
    return streak;
  }
  ...
  String get getSmartInsight {
    ...
    // Count ALL entries to match the Pie Chart
  ...
  Future<void> loadEntries() async {
    ...
    if (user == null) { _entries = []; notifyListeners(); return; }
    final box = await Hive.openBox<MoodEntry>(_boxName);
    _entries = box.values.toList()..sort((a, b) => b.timestamp.compareTo(a.timestamp));
    notifyListeners();
    try {
      final response = await _supabase.from('mood_entries').select().eq('user_id', user.id).order('timestamp', ascending: false);
      final cloudEntries = (response as List).map((data) => MoodEntry(
        emoji: data['emoji'], note: data['note'] ?? '', timestamp: DateTime.parse(data['timestamp']), voiceNotePath: data['voice_note_path'],
      )).toList();
      await box.clear(); await box.addAll(cloudEntries);
  ...
  Future<void> addEntry(MoodEntry entry) async {
    ...
    try {
      await _supabase.from('mood_entries').insert({
        'user_id': user.id, 'emoji': entry.emoji, 'note': entry.note,
        'timestamp': entry.timestamp.toIso8601String(), 'voice_note_path': entry.voiceNotePath,
      });
  ...
  Future<void> updateEntry(int index, String newEmoji, String newNote) async {
    ...
    final updatedEntry = MoodEntry(emoji: newEmoji, note: newNote, timestamp: entry.timestamp, voiceNotePath: entry.voiceNotePath);
    await box.put(hiveKey, updatedEntry);
    _entries[index] = updatedEntry;
    notifyListeners();
    try {
      await _supabase.from('mood_entries').update({'emoji': newEmoji, 'note': newNote})
          .eq('user_id', user.id).eq('timestamp', entry.timestamp.toIso8601String());
    } catch (e) { debugPrint('Cloud Update Error: $e'); }
  }
  ...
  Future<void> deleteEntry(int index) async {
    ...
    notifyListeners();
    try {
      await _supabase.from('mood_entries').delete()
          .eq('user_id', user.id).eq('timestamp', entry.timestamp.toIso8601String());
    } catch (e) { debugPrint('Cloud Delete Error: $e'); }
  }
  ...
  Future<void> deleteAllData() async {
    ...
    if (user == null) return;
    final box = await Hive.openBox<MoodEntry>(_boxName);
    await box.clear(); _entries = []; notifyListeners();
  ...
  Future<void> exportToPdf() async {
    ...
    pdf.addPage(pw.MultiPage(build: (context) => [
      ...
      pw.TableHelper.fromTextArray(headers: ['Date', 'Mood', 'Note'], data: _entries.map((e) => [DateFormat('yyyy-MM-dd HH:mm').format(e.timestamp), e.emoji, e.note]).toList()),
    ]));
